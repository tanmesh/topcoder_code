...
class NumberofFiboCalls {
    ...
    vector<int> fiboCallsMade(int n) {
        ...
        for(int i=2; i<=n; ++i) {
            int total_zero = a[i-1].first + a[i-2].first;
            int total_one = a[i-1].second + a[i-2].second;
            a.push_back(make_pair(total_zero, total_one));
        }
        vector<int> ans;
...
// CUT begin
ifstream data("NumberofFiboCalls.sample");
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "NumberofFiboCalls (500 Points)" << endl << endl;
    }
