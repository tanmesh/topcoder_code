...
// CUT begin
ifstream data("DistinctGridEasy.sample");
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(n, k, grid, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1503654621;
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "DistinctGridEasy (250 Points)" << endl << endl;
    }
