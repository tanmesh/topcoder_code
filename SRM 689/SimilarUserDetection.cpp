...
class SimilarUserDetection {
    ...
    string haveSimilar(vector<string> handles) {
        for(int i=0;i<handles.size();++i){
            int j=0;
            while(j<handles[i].length()){
                if(handles[i][j]=='O' || handles[i][j]=='0'){
                    handles[i][j]='@';
                }
                if(handles[i][j]=='1' || handles[i][j]=='l' || handles[i][j]=='I'){
                    handles[i][j]='#';
                }
                j++;
            }
        ...
        for(int i=0;i<handles.size();++i){
            ...
            str.insert(handles[i]);
        }
        if(str.size() == handles.size()){
